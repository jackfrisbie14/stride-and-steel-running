...
const TEST_EMAIL = "test-racegoal-api@test.com";
...
async function main() {
  ...
  // 2. Get CSRF token from NextAuth
  const csrfRes = await fetch(`${BASE_URL}/api/auth/csrf`);
  const { csrfToken } = await csrfRes.json();
  const cookies = csrfRes.headers.getSetCookie();
  console.log("2. Got CSRF token");

  // 3. Sign in with credentials to get session cookie
  ...
  // Collect all set-cookie headers from sign-in response
  ...
  // 5. POST to race-goal API to generate a race plan
  const raceDate = new Date();
  raceDate.setDate(raceDate.getDate() + 12 * 7); // 12 weeks out

  console.log("");
  console.log("=== Calling POST /api/user/race-goal ===");
  ...
  console.log("(This will call Claude Haiku 4.5 in batches...)");
  console.log("");
  ...
  console.log(`Response (${elapsed}s):`, JSON.stringify(raceGoalData, null, 2));
  console.log("");
  ...
  // 6. Verify GET race-goal returns plan info
  ...
  // 7. Verify workouts stored in DB
  ...
  const typeCounts = {};
  workouts.forEach((w) => {
    weekCounts[w.weekNumber] = (weekCounts[w.weekNumber] || 0) + 1;
    typeCounts[w.type] = (typeCounts[w.type] || 0) + 1;
  });
  console.log("Workouts per week:", JSON.stringify(weekCounts));
  console.log("By type:", JSON.stringify(typeCounts));
  console.log("");

  // Show week 1 workouts
  ...
  // 8. Test DELETE race goal (revert to quiz workouts)
  ...
  // Verify race mode deactivated
  const afterDelete = await prisma.user.findUnique({
    where: { email: TEST_EMAIL },
    select: { racePlanActive: true, raceName: true },
  });
  console.log("racePlanActive after delete:", afterDelete.racePlanActive);
  console.log("raceName after delete:", afterDelete.raceName);

  // Verify race workouts deleted and quiz workouts regenerated
  const raceWorkouts = await prisma.workout.count({ where: { userId: user.id, source: "race" } });
  const quizWorkouts = await prisma.workout.count({ where: { userId: user.id, source: "quiz" } });
  console.log(`Race workouts remaining: ${raceWorkouts}`);
  console.log(`Quiz workouts regenerated: ${quizWorkouts}`);
  console.log("");
